...
monthly_ppt <- mutate(monthly_ppt, date = paste(year,"-",month, sep = ""))
# Convert date factor to `date` format for plotting
monthly_ppt$date <- as.Date(paste(monthly_ppt$date, "-01", sep=""))
```
...
mp + geom_line() + facet_grid(id ~ ., scales = "fixed") + 
...
ggsave("figures/mp_id_facet.svg", width = 6, height = 10)
```
