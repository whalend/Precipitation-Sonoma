...
```{r dist 2 pacific coast, echo=F}
pac_coast <- readOGR("shapefiles/", "soco_pac_coast", verbose = F)
all.equal(proj4string(pac_coast), proj4string(stations_0308))
cal_bnd <- readOGR("shapefiles/", "CA_BOUNDARY", verbose = F)
(cal_bnd)
all.equal(proj4string(cal_bnd), proj4string(stations_0308))
plot(cal_bnd); plot(stations_0308, add = T)
plot(pac_coast); plot(stations_0308, add = T)

library(rgeos)
stations_0308@data$d2pacific <- gDistance(pac_coast, stations_0308, byid = T)
stations_0308@data$d2coast <- gDistance(cal_bnd, stations_0308, byid = F)

names(stations_0308@data)
cor(stations_0308@data$d2pacific, stations_0308@data$d2coast)
