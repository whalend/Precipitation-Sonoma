...
```{r read in rain gauge shapefiles}
...
stations_0308@data <- stations_0308@data %>% 
      select(-NUM) %>% 
      rename(station = NAME, lat_utm = NORTHING, lon_utm = EASTING)
...
stations_0911.dat <- stations_0911@data
stations_0911 <- stations_0911[,2:4]
summary(stations_1214)

stations_0911 <- spRbind(stations_0911, glen_ellen_rg)
stations_0911@data <- left_join(stations_0911@data, stations_0911.dat)
stations_1214 <- spRbind(stations_1214, glen_ellen_rg[, c("station", "lat_utm", "lon_utm")])
```

Join rainfall data to rain gauge shapefiles.
```{r join data}
summary(stations_0308)
stations_0308@data <- left_join(stations_0308@data, select(wide_monthly, stationid, ppt_nov03:ppt_may08), by = c("station" = "stationid"))
summary(stations_0308)

summary(stations_0911)
stations_0911@data <- left_join(stations_0911@data, select(wide_monthly, stationid, ppt_jun08:ppt_may11), by = c("station" = "stationid"))
summary(stations_0911)

summary(stations_1214)
stations_1214@data <- left_join(stations_1214@data, select(wide_monthly, stationid, ppt_jun11:ppt_may14), by = c("station" = "stationid"))
summary(stations_1214)
```

Read in seasonal rain gauge shapefiles.
...
Add latitude and longitiude to the attribute table.
```{r add lat/long to attributes, echo=F}
rainy04@data$lon_utm <- rainy04@coords[,1]
rainy04@data$lat_utm <- rainy04@coords[,2]
rainy05@data$lon_utm <- rainy05@coords[,1]
rainy05@data$lat_utm <- rainy05@coords[,2]
rainy06@data$lon_utm <- rainy06@coords[,1]
rainy06@data$lat_utm <- rainy06@coords[,2]
rainy07@data$lon_utm <- rainy07@coords[,1]
rainy07@data$lat_utm <- rainy07@coords[,2]
rainy08@data$lon_utm <- rainy08@coords[,1]
rainy08@data$lat_utm <- rainy08@coords[,2]
rainy09@data$lon_utm <- rainy09@coords[,1]
rainy09@data$lat_utm <- rainy09@coords[,2]
rainy10@data$lon_utm <- rainy10@coords[,1]
rainy10@data$lat_utm <- rainy10@coords[,2]
rainy11@data$lon_utm <- rainy11@coords[,1]
rainy11@data$lat_utm <- rainy11@coords[,2]
rainy12@data$lon_utm <- rainy12@coords[,1]
rainy12@data$lat_utm <- rainy12@coords[,2]
rainy14@data$lon_utm <- rainy14@coords[,1]
rainy14@data$lat_utm <- rainy14@coords[,2]
```
...
soco_plots@data$d2pacific <- apply(gDistance(pac_coast, soco_plots, byid = T), MARGIN = 1, FUN = min)
...
soco_plots@data$d2coast <- apply(gDistance(cal_bnd, soco_plots, byid = T), MARGIN = 1, FUN = min)
```

```{r seasonal rain gauge PRISM correlation}
rs04 <- select(rainy04@data, stationid, lat_utm, lon_utm, elev_m, totppt_mm, psm_totppt, starts_with("d2")) %>% mutate(year = 2004)
rs05 <- select(rainy05@data, stationid, lat_utm, lon_utm, elev_m, totppt_mm, psm_totppt, starts_with("d2")) %>% mutate(year = 2005)
rs06 <- select(rainy06@data, stationid, lat_utm, lon_utm, elev_m, totppt_mm, psm_totppt, starts_with("d2")) %>% mutate(year = 2006)
rs07 <- select(rainy07@data, stationid, lat_utm, lon_utm, elev_m, totppt_mm, psm_totppt, starts_with("d2")) %>% mutate(year = 2007)
rs08 <- select(rainy08@data, stationid, lat_utm, lon_utm, elev_m, totppt_mm, psm_totppt, starts_with("d2")) %>% mutate(year = 2008)
rs09 <- select(rainy09@data, stationid, lat_utm, lon_utm, elev_m, totppt_mm, psm_totppt, starts_with("d2")) %>% mutate(year = 2009)
rs10 <- select(rainy10@data, stationid, lat_utm, lon_utm, elev_m, totppt_mm, psm_totppt, starts_with("d2")) %>% mutate(year = 2010)
...
deviation <- select(rs_totals, stationid, totppt_mm, psm_totppt, year) %>% 
...
pairs(select(rainy04@data, totppt_mm, rain04_dys, elev_m, lat_utm, lon_utm, d2coast, d2pacific),
...
elev_m.log <- log(rainy04@data$elev_m)
...
rstot04.lm <- lm(totppt_mm ~ elev_m + d2pacific, data = rainy04)
summary(rstot04.lm)
elev04.coeff <- rstot04.lm$coefficients[2]
d2pac04.coeff <- rstot04.lm$coefficients[3]

rsdys04.lm <- lm(rain04_dys ~ elev_m + d2pacific, data = rainy04)
summary(rsdys04.lm)
elevdys04.coeff <- rsdys04.lm$coefficients[2]
d2pacdys04.coeff <- rsdys04.lm$coefficients[3]
```

```{r RS 2005 OLS models, echo=F}
pairs(select(rainy05@data, totppt_mm, rain05_dys, elev_m, lat_utm, lon_utm, d2coast, d2pacific),
      upper.panel = panel.cor, diag.panel = panel.hist,
...
rstot05.lm <- lm(totppt_mm ~ elev_m + lon_utm, data = rainy05)
summary(rstot05.lm)
elev05.coeff <- rstot05.lm$coefficients[2]
lon_utm05.coeff <- rstot05.lm$coefficients[3]

rsdys05.lm <- lm(rain05_dys ~ elev_m + d2pacific + d2coast, data = rainy05)
summary(rsdys05.lm)
elevdys05.coeff <- rsdys05.lm$coefficients[2]
d2pacdys05.coeff <- rsdys05.lm$coefficients[3]
d2cstdys05.coeff <- rsdys05.lm$coefficients[4]
```

```{r RS 2006 OLS models, echo=F}
pairs(select(rainy06@data, totppt_mm, rain06_dys, elev_m, lat_utm, lon_utm, d2coast, d2pacific),
      upper.panel = panel.cor, diag.panel = panel.hist,
...
rstot06.lm <- lm(totppt_mm ~ elev_m + d2pacific + d2coast, data = rainy06)
summary(rstot06.lm)
elev06.coeff <- rstot06.lm$coefficients[2]
d2pac06.coeff <- rstot06.lm$coefficients[3]
d2coast06.coeff <- rstot06.lm$coefficients[4]

rsdys06.lm <- lm(rain06_dys ~ d2pacific + d2coast, data = rainy06)
summary(rsdys06.lm)
d2pacdys06.coeff <- rsdys06.lm$coefficients[2]
d2cstdys06.coeff <- rsdys06.lm$coefficients[3]
```

```{r RS 2007 OLS models}
pairs(select(rainy07@data, totppt_mm, rain07_dys, elev_m, lat_utm, lon_utm, d2coast, d2pacific),
      upper.panel = panel.cor, diag.panel = panel.hist,
...
rstot07.lm <- lm(totppt_mm ~ elev_m + d2pacific + d2coast, data = rainy07)
summary(rstot07.lm)
elev07.coeff <- rstot07.lm$coefficients[2]
d2pac07.coeff <- rstot07.lm$coefficients[3]
d2coast07.coeff <- rstot07.lm$coefficients[4]

rsdys07.lm <- lm(rain07_dys ~ elev_m + d2pacific + d2coast, data = rainy07)
summary(rsdys07.lm)
elevdys07.coeff <- rsdys07.lm$coefficients[2]
d2pacdys07.coeff <- rsdys07.lm$coefficients[3]
d2cstdys07.coeff <- rsdys07.lm$coefficients[4]
```

```{r RS 2008 OLS models}
pairs(select(rainy08@data, totppt_mm, rain08_dys, elev_m, lat_utm, lon_utm, d2coast, d2pacific),
      upper.panel = panel.cor, diag.panel = panel.hist,
...
rstot08.lm <- lm(totppt_mm ~ elev_m + d2pacific + d2coast, data = rainy08)
summary(rstot08.lm)
elev08.coeff <- rstot08.lm$coefficients[2]
d2pac08.coeff <- rstot08.lm$coefficients[3]
d2coast08.coeff <- rstot08.lm$coefficients[4]

rsdys08.lm <- lm(rain08_dys ~ d2coast, data = rainy08)
summary(rsdys08.lm)# extremely weak correlation between rainy days and potential predictors
d2cstdys08.coeff <- rsdys08.lm$coefficients[2]
```

```{r RS 2009 OLS models}
pairs(select(rainy09@data, totppt_mm, rain09_dys, elev_m, lat_utm, lon_utm, d2coast, d2pacific),
      upper.panel = panel.cor, diag.panel = panel.hist,
...
rstot09.lm <- lm(totppt_mm ~ elev_m + d2coast + d2pacific, data = rainy09)
summary(rstot09.lm)
elev09.coeff <- rstot09.lm$coefficients[2]
d2coast09.coeff <- rstot09.lm$coefficients[3]
d2pac09.coeff <- rstot09.lm$coefficients[4]

rsdys09.lm <- lm(rain09_dys ~ elev_m + d2coast + d2pacific, data = rainy09)
summary(rsdys09.lm)
elevdys09.coeff <- rsdys09.lm$coefficients[2]
d2cstdys09.coeff <- rsdys09.lm$coefficients[3]
d2pacdys09.coeff <- rsdys09.lm$coefficients[4]
```

```{r RS 2010 OLS models}
pairs(select(rainy10@data, totppt_mm, rain10_dys, elev_m, lat_utm, lon_utm, d2coast, d2pacific),
      upper.panel = panel.cor, diag.panel = panel.hist,
...
rstot10.lm <- lm(totppt_mm ~ elev_m + d2coast + d2pacific, data = rainy10)
summary(rstot10.lm)
elev10.coeff <- rstot10.lm$coefficients[2]
d2coast10.coeff <- rstot10.lm$coefficients[3]
d2pac10.coeff <- rstot10.lm$coefficients[4]

rsdys10.lm <- lm(rain10_dys ~ elev_m + d2coast + d2pacific, data = rainy10)
summary(rsdys10.lm)
elevdys10.coeff <- rsdys10.lm$coefficients[2]
d2cstdys10.coeff <- rsdys10.lm$coefficients[3]
d2pacdys10.coeff <- rsdys10.lm$coefficients[4]
```
...
rstot11.lm <- lm(totppt_mm ~ elev_m + d2coast + d2pacific, data = rainy10)
summary(rstot11.lm)
elev11.coeff <- rstot11.lm$coefficients[2]
d2coast11.coeff <- rstot11.lm$coefficients[3]
d2pac11.coeff <- rstot11.lm$coefficients[4]

rsdys11.lm <- lm(rain11_dys ~ elev_m + d2pacific, data = rainy11)
summary(rsdys11.lm)
elevdys11.coeff <- rsdys11.lm$coefficients[2]
d2pacdys11.coeff <- rsdys11.lm$coefficients[3]
```
...
rstot12.lm <- lm(totppt_mm ~ elev_m + d2pacific + d2coast, data = rainy12)
summary(rstot12.lm)
elev12.coeff <- rstot12.lm$coefficients[2]
d2pac12.coeff <- rstot12.lm$coefficients[3]
d2coast12.coeff <- rstot12.lm$coefficients[4]

rsdys12.lm <- lm(rain12_dys ~ elev_m + d2pacific, data = rainy12)
summary(rsdys12.lm)
elevdys12.coeff <- rsdys12.lm$coefficients[2]
d2pacdys12.coeff <- rsdys12.lm$coefficients[3]
```
...
rstot14.lm <- lm(totppt_mm ~ elev_m + d2pacific, data = rainy14)
summary(rstot14.lm)
elev14.coeff <- rstot14.lm$coefficients[2]
d2pac14.coeff <- rstot14.lm$coefficients[3]

rsdys14.lm <- lm(rain14_dys ~ d2pacific + d2coast, data = rainy14)
summary(rsdys14.lm)
d2pacdys14.coeff <- rsdys14.lm$coefficients[2]
d2cstdys14.coeff <- rsdys14.lm$coefficients[3]
```

```{r regression rainfall/rainy days estimates}
summary(soco_plots)
soco_plots$ppt04tot_r <- with(soco_plots@data, elevation*elev04.coeff + d2coast*d2pac04.coeff)
soco_plots$ppt04dys_r <- with(soco_plots@data, elevation*elevdys04.coeff + d2pacific*d2pacdys04.coeff)

soco_plots$ppt05tot_r <- with(soco_plots@data, elevation*elev05.coeff + X*lon_utm05.coeff)# wildly overpredicts the values
soco_plots$ppt05dys_r <- with(soco_plots@data, elevation*elevdys05.coeff + d2coast*d2cstdys05.coeff + d2pacific*d2pacdys05.coeff)# all negative relationships, so can only get negative predicted values

soco_plots$ppt06tot_r <- with(soco_plots@data, elevation*elev06.coeff + d2pacific*d2pac06.coeff + d2coast*d2coast06.coeff)
soco_plots$ppt06dys_r <- with(soco_plots@data, d2coast*d2cstdys06.coeff + d2pacific*d2pacdys06.coeff)

soco_plots$ppt07tot_r <- with(soco_plots@data, elevation*elev07.coeff + d2pacific*d2pac07.coeff + d2coast*d2coast07.coeff)
soco_plots$ppt07dys_r <- with(soco_plots@data, elevation*elevdys07.coeff + d2pacific*d2pacdys07.coeff + d2coast*d2cstdys07.coeff)

soco_plots$ppt08tot_r <- with(soco_plots@data, elevation*elev08.coeff + d2pacific*d2pac08.coeff + d2coast*d2coast08.coeff)
soco_plots$ppt08dys_r <- with(soco_plots@data, d2coast*d2cstdys08.coeff)

soco_plots$ppt09tot_r <- with(soco_plots@data, elevation*elev09.coeff + d2pacific*d2pac09.coeff + d2coast*d2coast09.coeff)
soco_plots$ppt09dys_r <- with(soco_plots@data, elevation*elevdys09.coeff + d2pacific*d2pacdys09.coeff + d2coast*d2cstdys09.coeff)

soco_plots$ppt10tot_r <- with(soco_plots@data, elevation*elev10.coeff + d2pacific*d2pac10.coeff + d2coast*d2coast10.coeff)
soco_plots$ppt10dys_r <- with(soco_plots@data, elevation*elevdys10.coeff + d2pacific*d2pacdys10.coeff + d2coast*d2cstdys10.coeff)

soco_plots$ppt11tot_r <- with(soco_plots@data, elevation*elev11.coeff + d2pacific*d2pac11.coeff + d2coast*d2coast11.coeff)
soco_plots$ppt11dys_r <- with(soco_plots@data, elevation*elevdys11.coeff + d2pacific*d2pacdys11.coeff)

soco_plots$ppt12tot_r <- with(soco_plots@data, elevation*elev12.coeff + d2pacific*d2pac12.coeff + d2coast*d2coast12.coeff)
soco_plots$ppt12dys_r <- with(soco_plots@data, elevation*elevdys12.coeff + d2pacific*d2pacdys12.coeff)

soco_plots$ppt14tot_r <- with(soco_plots@data, elevation*elev14.coeff + d2pacific*d2pac14.coeff)
soco_plots$ppt14dys_r <- with(soco_plots@data, d2pacific*d2pacdys14.coeff + d2coast*d2cstdys14.coeff)

summary(soco_plots@data)
# Quite a few of the estimates for rainy days end up being negative; in short, regression is a pretty terrible way to try and estimate number of rainy days using these data.
writeOGR(soco_plots, "shapefiles/", "plot202ppt_regression", "ESRI Shapefile", overwrite_layer = T)
```

```{r remove coefficient estimates}
rm(list = ls(pattern = "*coeff"))
```
...
summary(rg0308)
rg0308$lon_utm <- rg0308@coords[,1]
rg0308$lat_utm <- rg0308@coords[,2]
...
ppt04.sp <- rg0308[, c("stationid", "lat_utm", "lon_utm", "elev_m", "rain04_tot")]
ppt04.sp <- ppt04.sp[!is.na(ppt04.sp$rain04_tot), ]
summary(ppt04.sp)
filter(ppt04.sp@data, is.na(elev_m))
# ppt04.sp <- ppt04.sp[ppt04.sp$station != "fop", ]
ppt04.sp@data

plot(ppt04.sp, pch = 1, cex = log(ppt04.sp$rain04_tot))

ppt04.sp.lm <- lm(rain04_tot ~ lat_utm + lon_utm, data = ppt04.sp)
summary(ppt04.sp.lm)

max.dist <- 0.25 * max(iDist(ppt04.sp@coords))
bins <- 50

vario_rain04a <- variog(coords = ppt04.sp@coords, 
                          data = ppt04.sp$rain04_tot)
plot(vario_rain04a)
str(vario_rain04a)
mean(vario_rain04a$v)

fit.rain04a <- variofit(vario_rain04a,
                          #ini.cov.pars = c(862, 1000) ,
                          cov.model = "exponential",
                          minimisation.function = "nls",
                          weights = "equal")

vario.rain04b <- variogram(rain04_tot ~ elev_m, data = ppt04.sp[ppt04.sp$stationid != "petaluma_e",]
                             # cutoff = max.dist, 
                             # width = 5
                             )

fit.rain04b <- fit.variogram(vario.rain04b, vgm(psill = 1000, model = "Exp", 100/-log(.05), 600))
plot(fit.rain04b, vario.rain04b)
```
