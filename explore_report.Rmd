...
summary(hr_rg_data)
```
This grouping and summarization has eliminated `NA`s from the resulting data frame. Next I will `rbind` it with the `hourly_special` dataframe
...
# class(dy_rg_data)
```
Next, `rbind` this with the `daily_special` dataframe
...
library(ggplot2)
summary(dy_rg_data)
qplot(, daily_ppt, data=dy_rg_data) + theme_bw()
