...
hr_rg_data <- rg_data %>% 
      select(id, year, month, day, hour, events, date) %>% 
      group_by(id, year, month, day, hour, date) %>%
      summarise(hourly_events=length(events), hourly_ppt=length(events)*0.01)
hr_rg_data
# class(hr_rg_data)
summary(hr_rg_data)
```
...
hr_rg_data$date <- as.Date(hr_rg_data$date, format = "%m/%d/%Y")
```
...
dy_rg_data <- rg_data %>% 
      select(id, year, month, day, events, date) %>% 
      group_by(id, year, month, day, date) %>%
      summarize(daily_events=length(events), daily_ppt=length(events)*0.01)
dy_rg_data
...
For outlier replacement I settled on a daily precipitation threshold of greater than or equal to 6 inches, and an hourly precipitation threshold of greater than or equal to 1.5 inches.
